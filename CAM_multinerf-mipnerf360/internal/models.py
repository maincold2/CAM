...
def cart2sph(cartesian_coords):
    ...
    azi = jnp.arctan2(y, x) / pi / 2 + 0.5
    ele = jnp.arctan2(z, jnp.sqrt(x*x + y*y)) * (-2.0) / pi
...
class affine(nn.Module):
  res_azi: int = 10
  res_ele: int = 3
  gamma_init: Callable[[PRNGKey, Shape, Dtype], Array] = jax.nn.initializers.ones()
  beta_init: Callable[[PRNGKey, Shape, Dtype], Array] = jax.nn.initializers.zeros()
  ...
  def __call__(self, x, coord):
    ...
    sh = tuple(x.shape[0] if i==0 else 1 for i in range(len(x.shape)))
    x = jax.scipy.ndimage.map_coordinates(gamma, coord, 1, 'nearest').reshape(sh)*x + jax.scipy.ndimage.map_coordinates(beta, coord, 1, 'nearest').reshape(sh)
    return x
  ...
  def __call__(self,
    ...
    def predict_density(means, covs):
      ...
      for i in range(self.net_depth):
        ...
        if x.shape[1] == 256:
            x = affine_layer()(x, cart2sph(viewdirs.reshape(-1,3)))

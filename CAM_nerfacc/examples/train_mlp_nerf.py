...
optimizer = torch.optim.Adam([{'params':net, 'lr':5e-4}, {'params':grid, 'lr':5e-3}])
...
for step in range(max_steps + 1):
    ...
    # compute loss
    loss = F.smooth_l1_loss(rgb, pixels) #+ 1e-5/radiance_field.qloss
    ...
    if step > 0 and step % max_steps == 0:
        model_save_path =  os.path.join(path,f"mlp_nerf_{step}")
        torch.save(
            {
                "step": step,

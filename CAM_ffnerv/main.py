...
def main():
    ...
    parser.add_argument('--act', type=str, default='gelu', help='activation to use', choices=['relu', 'leaky', 'leaky01', 'relu6', 'gelu', 'swish', 'softplus', 'hardswish', 'none'])
    ...
    parser.add_argument('--mod-dim', default=60, type=int, help='temporal resolution of modulation grids')
...
def train(local_rank, args):
    ...
    model = Generator(fc_hw_dim=args.fc_hw_dim, expansion=args.expansion, 
        ...
        stride_list=args.strides,  sin_res=args.single_res,  lower_width=args.lower_width, sigmoid=args.sigmoid, aggs=len(args.agg_ind), wbit=args.wbit, t_dim=args.t_dim, mod_dim=args.mod_dim)
...
def evaluate(model, train_dataloader, val_dataloader, local_rank, args):
    ...
    if args.eval_only and args.wbit!=32:
        if args.wbit == 6:
            ...
            for k,v in cur_ckt.items():
                #applying qfn
                if k.split('.')[0] == 'head_layers':
                    head_params += v.nelement()
                else:
                    weight = torch.tanh(v)
                    quant_v = qfn.apply(weight, args.wbit)
                    if args.prune_ratio < 1.0:
                        valid_quant_v = quant_v[quant_v!=0]
                    else:
        ...
        print(len(input_code_list) / 1e6, head_params / 1e6)
        # import pdb; pdb.set_trace; from IPython import embed; embed()       
        encoding_efficiency = avg_bits / args.wbit
        print_str = f'{args.wbit}bit: {len(input_code_list)} head: {head_params} \n'
        print_str = print_str + f'Entropy encoding efficiency for bit {args.wbit}: {encoding_efficiency}'
        print(print_str)
        if local_rank in [0, None]:
            with open('{}/eval.txt'.format(args.outf), 'a') as f:
                f.write(print_str + '\n')       
